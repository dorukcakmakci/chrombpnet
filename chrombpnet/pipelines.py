...
"""
Bias-factorized (2 stage) ChromBPNet pipeline
1) Pretrained bias model (chrombpnet.pipelines.train_bias_pipeline)
2) Train Bias-factorized ChromBPNet model (chrombpnet.pipelines.chrombpnet_train_pipeline)
"""
...
"""
Single-stage ChromBPNet model
(No trained bias model used)
"""

def single_stage_chrombpnet_train_pipeline(args):
	...
	if args.input_bigwig:
		# Copy input bigwig to expected location to maintain downstream compatibility
		import shutil
		expected_bigwig_path = os.path.join(args.output_dir,"auxiliary/{}data_unstranded.bw".format(fpx))
		shutil.copy2(args.input_bigwig, expected_bigwig_path)
		args.bigwig = expected_bigwig_path
	else:
		# Original BAM/fragment/tagAlign → bigwig conversion
		import chrombpnet.helpers.preprocessing.reads_to_bigwig as reads_to_bigwig	
		args.output_prefix = os.path.join(args.output_dir,"auxiliary/{}data".format(fpx))
		args.plus_shift = None
		args.minus_shift = None
	...
	# fetch hyperparameters for training
	import chrombpnet.helpers.hyperparameters.find_single_stage_chrombpnet_hyperparams as find_single_stage_chrombpnet_hyperparams
	args_copy = copy.deepcopy(args)
	args_copy.output_prefix = os.path.join(args.output_dir,"auxiliary/{}".format(fpx))
	find_single_stage_chrombpnet_hyperparams.main(args_copy)
	
	# NOTE: No bias model for this model
	# # make predictions with input bias model in peaks
	# import chrombpnet.training.predict as predict
	# args_copy = copy.deepcopy(args)
	# args_copy.output_prefix = os.path.join(args_copy.output_dir,"evaluation/bias")
	...
	# get model architecture path and train chromBPNet model
	import chrombpnet.training.models.single_stage_chrombpnet_model as single_stage_chrombpnet_model
	import chrombpnet.training.train as train
	args_copy = copy.deepcopy(args)
	if args_copy.architecture_from_file is None:
		args_copy.architecture_from_file = 	single_stage_chrombpnet_model.__file__
	args_copy.peaks = os.path.join(args.output_dir,"auxiliary/{}filtered.peaks.bed".format(fpx))
	args_copy.nonpeaks = os.path.join(args.output_dir,"auxiliary/{}filtered.nonpeaks.bed".format(fpx))
	args_copy.output_prefix = os.path.join(args.output_dir,"models/{}chrombpnet".format(fpx))
	args_copy.params = os.path.join(args.output_dir,"logs/{}chrombpnet_model_params.tsv".format(fpx))
...
def single_stage_chrombpnet_qc(args):
...
""" Short (7bp) and Long (50bp) input kernel single stage chrombpnet model training pipelines"""


""" Short"""

def single_stage_short_kernel_chrombpnet_train_pipeline(args):
	...
	# get model architecture path and train chromBPNet model
	import chrombpnet.training.models.single_stage_short_kernel_chrombpnet_model as single_stage_chrombpnet_model
	import chrombpnet.training.train as train
	args_copy = copy.deepcopy(args)
	if args_copy.architecture_from_file is None:
		args_copy.architecture_from_file = 	single_stage_chrombpnet_model.__file__
...
def single_stage_long_kernel_chrombpnet_train_pipeline(args):
	...
	# get model architecture path and train chromBPNet model
	import chrombpnet.training.models.single_stage_long_kernel_chrombpnet_model as single_stage_chrombpnet_model
	import chrombpnet.training.train as train
	args_copy = copy.deepcopy(args)
	if args_copy.architecture_from_file is None:
		args_copy.architecture_from_file = 	single_stage_chrombpnet_model.__file__
